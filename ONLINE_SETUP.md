...
仓库发布后，`Smart industry update` 每 3 小时第 17 分钟自动运行一次。选择非整点时间可降低 GitHub 调度拥堵，实际启动可能有少量延迟。当前覆盖六个板块、32 个公开来源，其中包含 11 个中国权威来源；采集方式支持 RSS / Atom、官方网页栏目和官方 JSON。平台资料池只保留 45 天内且有明确来源日期的线索，并按来源日期、板块相关度和来源可信度排序。任务优先利用 GitHub Models 免费额度生成来源约束的中文原创整理，模型临时不可用时使用透明的来源约束结构化整理；只有正文不少于 800 字且通过来源、结构、重复、图片和可信度检查的内容才能自动公开，其余内容保留在待审核区。
